...
public class Window {
    ...
    public void run() {
        ...
        // free the memory - we are using C bindings in GL
    ...
    public void init() {
        ...
        // set mouse callbacks using method references (lambdas)
        glfwSetCursorPosCallback(glfwWindow, MouseListener::mousePosCallback);
        glfwSetMouseButtonCallback(glfwWindow, MouseListener::mouseButtonCallback);
        glfwSetScrollCallback(glfwWindow, MouseListener::mouseScrollCallback);

        // set key callback listeners
    ...
    public void loop() {
        while(!glfwWindowShouldClose(glfwWindow)) {
            ...
            glClearColor(r, g, b, a); //white
            glClear(GL_COLOR_BUFFER_BIT);

            if (fadeToBlack) {
                r = r - Math.max(0.01f, 0);
                g = g - Math.max(0.01f, 0);
                b = b - Math.max(0.01f, 0);
            }
